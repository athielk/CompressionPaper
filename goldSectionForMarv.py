import math, random, time, sys, glob
import pygame
...
class golden_section_queue(object):
    ...
    def update(self, result):
        ...
        if result == 1:  # update lower
            ...
            raise ValueError(
                "result %i not expected only excepts 1,2" % (result))
...
class mixed_queue(object):
    ...
    def next(self):
        ...
        for _ in range(len(self.queues)):
            ...
            if not self.queues[randomIndex].complete:
                self.next_queue = randomIndex
                return self.queues[self.next_queue].next()
            else:
                print "randomIndex %i is complete"
...
def run():
    ...
    queue1 = golden_section_queue(
        glob.glob('bush*.wav'), 'bushOffersPeace.wav')
    queue2 = golden_section_queue(
        glob.glob('bush*.wav'), 'bushOffersPeace.wav')
